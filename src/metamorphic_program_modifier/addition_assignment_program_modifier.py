...
class AdditionAssignmentProgramModifier(MetamorphicProgramModifier):
    """
    This class modifies a base program based on the addition symbol metamorphic relation,
    which states that translating i += x to i = i + x in a program should not affect
    its semantics.
    ...
    def convert_addition_assignment(self, original_program):
        """
            Converts the += in the given program

            Parameters:
                original_program: The program to be modified
        """
        ...
        for line in lines:  # find all instances in program
            if '+=' in line:
                ...
                before_var = self.find_substring_before_target(line.split('+=')[0],
                                                               var_name)
                increment_val = line.split('+=')[1].strip() # remove additional spaces
                string_add = f"{before_var}{var_name} = {var_name.strip()} + {increment_val}"
                modified_lines.append(string_add)
            else:
    ...
    def find_substring_before_target(self, s, target):
        """
            Helper function that gets everything before target in s
